#' Add AGE using OUTRIDER method
#'
#' @description Injects AGE (Aberrant Gene Expression) in the raw count.
#'
#' @param SequencingTable a read counts table with the transcripts in row and the samples in column.
#' @param SizeFactor vector containing all the size factor. The size factor of each sample can be computed using the function SizeFactors, the function use the estimatation of DESeq2.
...
#' @param drawoption the way to pick AGE, if the value is smaller than 1 it is the probability of observation gets noisy. If it is higher than one it is the number of observations to change.
#' @param feedback logical, add a data frame containing informations on the AGE
#' @param setseed set the seed.
#'
#' @return A sequencing table with AGE. AGE are drawn using the formula:
#' \ifelse{html}{
...
#' @export
AddOutriderAGE <- function(SequencingTable, SizeFactor, Zmean = log(3), Zsd = log(1.6), drawoption = 10^-5, feedback = FALSE, setseed = FALSE){
  ...
  part2 <- 2**(uij + part1)
  onlyAGE <- round(sweep(part2, 2, SizeFactor, FUN = '*'))
  original_value <- c()
  transformed_value <- c()
  for (i in 1:length(random_row)){
    original_value <- c(original_value,SequencingTable[random_row[i],random_col[i]])
    TableWithAGE[random_row[i],random_col[i]] <- min(max(SequencingTable)+1,onlyAGE[random_row[i],random_col[i]])
    transformed_value <- c(transformed_value,TableWithAGE[random_row[i],random_col[i]])
  }
  if (feedback == FALSE){
    return(TableWithAGE)
  } else {
    ...
    listreturn <- list('table' = TableWithAGE, 'feedback' = feedbackdata)
    return(listreturn)
  }
}
