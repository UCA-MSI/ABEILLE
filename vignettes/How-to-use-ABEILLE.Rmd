...
author: "Gwendal Le Bideau, David Pratella, Samira Ait-El-Mkadem - Saadi, Sylvie Bannwarth, Véronique Paquis-Flucklinger, Silvia Bottini"
...
Then two metric is needed for the linear regression, ABEILLE offers a function to compute the Divergence score and the Delta count. As the neural network you can provide your own metric.
```{r}
...
The *PickOutliers* function will also use the function *ComputeAberrantScore* that will return an Aberrant Score associated to each Outliers with a score from 1 to 4.
```{r}
outliers <- PickOutliers(divergence_score, delta_count, toy_dataset, toy_dataset_recons)
print(head(outliers))
```
...
outliers <- PickOutliers(divergence_score, delta_count, toy_dataset, toy_dataset_recons, decision_tree = light_tree)
print(head(outliers))
```
